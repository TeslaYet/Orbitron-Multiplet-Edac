...
int main( int argc, char *argv[] )
{
 ...
 /* if (-chimin > chimax ) chimax = -chimin ; else chimin = -chimax ; */
  chivar = ( chimax - chimin ) / 33.999999999999 ;
...
*/
  ...
  sprintf(pngfile,"%s_%.0lf.png",argv[1],1.e4*chimax) ;
  fp = fopen("temp.xpm","w") ;
  fprintf(fp,"/* XPM */\n") ;
  fprintf(fp,"/* Blue: %lf White: 0  Red: %lf */\n", chimin, chimax ) ;
  fprintf(fp,"static char * pict[]= {\n\"") ;
  fprintf(fp,"%d %d %d %d\",\n", npoints, npoints, 32, 1 ) ;
  for ( i = 0, j = 0 ; i < 16 ; i++, j++ ) 
    fprintf(fp,"\"%c c #%2s%2sff \",", ( j<26 ? 'a'+j : 'A'+j-26 ), col16[i],col16[i] ) ;
  fprintf(fp,"\n") ;
  for ( i = 0 ; i < 15 ; i++, j++ )
    fprintf(fp,"\"%c c #ff%2s%2s \",", ( j<26 ? 'a'+j : 'A'+j-26 ), col16[14-i],col16[14-i] ) ;
  fprintf(fp,"\"F c None\"\n}\n") ;
...
*/
	...
	if ( chichi < -15. ) ichichi = -15 ;
        else if ( chichi > 15. ) ichichi = 15 ;
        else ichichi = (int) (chichi + 15.5) -15 ;
      }
      /* else if ( fabs( th - 90. ) < .65/POINTSPERDEG ) ichichi = 0 ; */
      /* else ichichi = 0 ; */
      else ichichi = 16 ;
      chiline[ jpt ] = ichichi + 15 ;
/*      printf("th = %lf, ph = %lf, chi = %lf\n", th, ph, chichi ) ; */
  ...
  fprintf(fp,"} ;\n") ;
  fclose(fp) ;
  sprintf(command,"convert temp.xpm %s", pngfile ) ;
  system(command) ;
